        ...
        <div class="m-portlet__head-caption">
          <div class="m-portlet__head-title">
            <h3 class="m-portlet__head-text">
              {{ lang('adCampaigns.createCampaign') }}
            </h3>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions('campaigns', ['createCampaign']),
    /**
     * Validate the current form data
     * Trigger the createRecord method if the data is valid
     */
    validateRequest() {
      this.validate(this.createRecord);
    },
    createRecord() {
      this.createCampaign(this.$data.campaign)
        .then(() => {
          this.success('adCampaigns.storeSuccess')
            .then(() => this.redirectTo('/advertising/campaigns/active'));
        })
        .catch(() => this.fail('adCampaigns.storeFail'));
    },
  },
};
</script>
